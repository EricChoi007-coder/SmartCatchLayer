...
/// <summary>
/// 混合缓存配置服务：自动扫描所有 Controller/Action，加载 Attribute 和 appsettings.json 配置
/// </summary>
public class HybridCacheConfigurationService
{
    ...
    public CacheConfigurationResult GetCacheConfig(string path)
    {
        ...
        // 3. Controller 通配符匹配（如 /api/products/*）
    ...
    private void DiscoverAllConfigurations()
    {
        ...
        _logger.LogInformation("混合缓存配置加载完成: {Count} 个端点配置", _cacheConfigs.Count);
    }
    ...
    private void MergeAppSettingsConfigurations()
    {
        ...
        foreach (var kvp in _appSettingsConfig.PathConfigs)
        {
            ...
            {
                // Attribute 优先级更高，但如果 Attribute 没有设置 Duration，则补上
        ...
        // 处理 Controller 通配符配置（如 /api/products/*）
        foreach (var kvp in _appSettingsConfig.PathConfigs)
        {
            if (kvp.Key.EndsWith("/*"))
            {
    ...
    private string? GetControllerKey(string path)
    {
        ...
        if (parts.Length >= 2)
        {
            var controller = parts[1]; // /api/products/... -> products
            return $"/{controller}/*";
        }
        return null;
    }
